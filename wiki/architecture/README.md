...
| [item-authoring](item-authoring.md) | Content-author contract for catalog item GLBs: `slot_` material naming, authored defaults + `aruct_material` extras, the `cutout` reserved mesh, UV world scale, and the validated Blender/export recipe |
...
| [viewer-isolation](viewer-isolation.md) | Keeping `@aruct/viewer` editor-agnostic |
