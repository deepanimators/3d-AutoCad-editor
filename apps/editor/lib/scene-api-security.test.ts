...
afterEach(() => {
  restoreEnv('ARUCT_SCENE_API_TOKEN')
  restoreEnv('ARUCT_SCENE_API_ORIGINS')
  restoreEnv('ARUCT_SCENE_API_RATE_LIMIT')
})
...
test('allows loopback scene API requests without a token', () => {
  delete process.env.ARUCT_SCENE_API_TOKEN
...
test('accepts bearer token auth when configured', () => {
  process.env.ARUCT_SCENE_API_TOKEN = 'secret'
  const request = new Request('https://editor.example/api/scenes', {
    headers: {
...
test('applies configured CORS origins for preflight', () => {
  process.env.ARUCT_SCENE_API_ORIGINS = 'https://app.example'
  const request = new Request('https://editor.example/api/scenes', {
    method: 'OPTIONS',
    headers: {
      host: 'editor.example',
