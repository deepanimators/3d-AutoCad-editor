...
function loadBuiltinsSync(): void {
  ...
  if (isDev()) {
    ...
    if (typeof console !== 'undefined') {
      console.info(
        `[aruct:registry] loaded ${builtinPlugin.id} v${builtinPlugin.apiVersion} (${kinds.length} kinds: ${kinds.join(', ') || '∅'})`,
      )
    }
...
export async function loadExternalPlugins(): Promise<void> {
  ...
  if (isDev() && externals.length > 0 && typeof console !== 'undefined') {
    console.info(`[aruct:registry] + ${externals.length} discovered plugin(s)`)
  }
}
