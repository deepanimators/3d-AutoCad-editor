    ...
    <svg className={className} width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <pattern id="lp-sm" width="50" height="50" patternUnits="userSpaceOnUse">
          <path d="M 50 0 L 0 0 0 50" fill="none" stroke="white" strokeWidth="0.4" strokeOpacity="0.06" />
        </pattern>
        <pattern id="lp-lg" width="250" height="250" patternUnits="userSpaceOnUse">
          <rect width="250" height="250" fill="url(#lp-sm)" />
          <path d="M 250 0 L 0 0 0 250" fill="none" stroke="white" strokeWidth="1" strokeOpacity="0.09" />
        </pattern>
      </defs>
...
function Navbar() {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${solid ? 'bg-[#06080f]/92 backdrop-blur-xl border-b border-white/[0.07]' : 'border-b border-transparent'}`}>
      <div className="mx-auto flex max-w-6xl items-center gap-6 px-6 py-4">
        ...
        <div className="ml-auto flex items-center gap-3">
          <Link href="/login" className="hidden text-[13px] text-white/50 hover:text-white transition-colors sm:block">Sign in</Link>
    ...
    >
      ...
      <div className="relative rounded-2xl border border-white/[0.1] bg-[#07091a] shadow-[0_0_80px_rgba(59,107,255,0.08)] overflow-hidden">
        ...
        <svg viewBox="0 0 440 360" className="w-full" xmlns="http://www.w3.org/2000/svg">
          ...
          {/* ── Internal walls ── */}
          {/* Vertical divider x=180: y=40→75 gap 75→105 y=105→300 */}
          ...
          {/* Horizontal divider y=160: x=20→100 gap 100→130 x=130→320 */}
...
function HeroSection() {
  ...
  return (
    <section ref={ref} className="relative flex min-h-screen items-center overflow-hidden bg-[#06080f]">
      ...
      <motion.div className="pointer-events-none absolute inset-0" style={{ y: glowY }}>
        <div className="absolute -top-32 left-1/4 h-[700px] w-[700px] rounded-full bg-blue-700/[0.1] blur-[140px]" />
        <div className="absolute top-1/2 right-0 h-[400px] w-[500px] -translate-y-1/2 rounded-full bg-indigo-700/[0.06] blur-[100px]" />
      </motion.div>

      <div className="pointer-events-none absolute bottom-0 inset-x-0 h-56 bg-gradient-to-t from-[#06080f] to-transparent" />
      ...
      >
        ...
        <motion.div className="text-center lg:text-left" style={{ y: contentY }}>
          <motion.div
            className="mb-7 inline-flex items-center gap-2 rounded-full border border-blue-500/25 bg-blue-500/[0.08] px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.22em] text-blue-300/80"
          ...
          <motion.p
            className="mx-auto mb-8 max-w-sm text-[16px] leading-[1.65] text-white/48 lg:mx-0"
          ...
          >
            ...
            <Link href="/editor" className="flex w-full items-center justify-center rounded-xl border border-white/[0.1] bg-white/[0.04] px-6 py-3.5 text-[14px] font-semibold text-white/60 hover:bg-white/[0.08] hover:text-white transition-all sm:w-auto">
          ...
          >
            {CHIPS.map((c) => (
              <span key={c} className="rounded-full border border-white/[0.07] bg-white/[0.02] px-3 py-1 text-[11.5px] font-medium text-white/38">
                {c}
              </span>
            ))}
          </motion.div>
      ...
      >
        <motion.div
          ...
          className="flex h-10 w-6 items-start justify-center rounded-full border border-white/18 pt-1.5"
        >
          <div className="h-2 w-1 rounded-full bg-white/38" />
        </motion.div>
      </motion.div>
    </section>
  )
...
function FeaturesSection() {
  ...
  return (
    <motion.section ref={sectionRef} className="bg-[#06080f] px-6 py-24 sm:py-32" style={{ y: sectionY }}>
...
function PluginCard({ card, index, inView }: { card: (typeof PLUGIN_CARDS)[0]; index: number; inView: boolean }) {
  ...
  return (
    <motion.div
      className="group relative overflow-hidden rounded-2xl border border-white/[0.07] bg-[#0a0d1c] p-6 cursor-default"
    ...
    >
      {/* Card shine on hover */}
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-white/[0.04] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      ...
      <h3 className="relative mb-1.5 text-[14px] font-semibold text-white">{card.name}</h3>
      <p className="relative text-[12.5px] leading-relaxed text-white/42">{card.desc}</p>
...
function PluginShowcase() {
  ...
  return (
    <motion.section ref={sectionRef} className="relative overflow-hidden bg-[#06080f] border-y border-white/[0.07] px-6 py-24 sm:py-32" style={{ y: sectionY }}>
      {/* Background glow */}
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-[600px] w-[800px] rounded-full bg-blue-900/[0.12] blur-[120px]" />
      </div>
      {/* Grid overlay */}
      ...
      <div className="relative mx-auto max-w-6xl">
        ...
        <div ref={headerRef} className="mb-14 flex flex-col items-center gap-5 text-center lg:flex-row lg:items-end lg:justify-between lg:text-left">
          <div>
            <motion.p
              className="mb-3 text-[11px] font-semibold uppercase tracking-[0.2em] text-blue-400/70"
              initial={{ opacity: 0, y: 20 }}
            ...
            <motion.p
              className="mt-3 max-w-md text-[15px] text-white/45"
              initial={{ opacity: 0, y: 20 }}
              animate={headerInView ? { opacity: 1, y: 0 } : {}}
          ...
          >
            <Link href="/plugins" className="group inline-flex items-center gap-2 rounded-xl border border-white/[0.1] bg-white/[0.04] px-5 py-2.5 text-[13px] font-semibold text-white/60 hover:bg-white/[0.08] hover:text-white transition-all">
              Browse all plugins
        ...
        {/* Plugin grid with 3D tilt on hover */}
  ...
  return (
    <motion.section ref={sectionRef} className="bg-[#06080f] px-6 py-24 sm:py-32" style={{ y: sectionY }}>
      <div className="mx-auto max-w-6xl">
        ...
        <div
          ref={ref}
          className="grid overflow-hidden rounded-2xl border border-white/[0.07] bg-white/[0.05] sm:grid-cols-2 lg:grid-cols-4"
          style={{ gap: 1 }}
        >
          {STEPS.map((s, i) => (
            <motion.div
              key={s.n}
              className="group relative bg-[#06080f] p-8 overflow-hidden"
              initial={{ opacity: 0, y: 28 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.65, delay: i * 0.1, ease: [0.22, 1, 0.36, 1] }}
            >
              {/* Large bg number */}
              <div className="absolute -top-4 -left-1 select-none font-black leading-none tracking-[-0.05em] text-white/[0.04] pointer-events-none" style={{ fontSize: 120 }}>
                {s.n}
              </div>
              <div className="relative mb-5 inline-flex h-8 w-8 items-center justify-center rounded-lg bg-blue-500/10 text-[12px] font-bold text-blue-400">
                {s.n}
              </div>
              <h3 className="relative mb-2 text-[15px] font-semibold text-white">{s.title}</h3>
              <p className="relative text-[13px] leading-relaxed text-white/45">{s.body}</p>
            </motion.div>
          ))}
        </div>
      </div>
...
function CTASection() {
  ...
  return (
    <motion.section ref={sectionRef} className="relative overflow-hidden bg-[#06080f] px-6 py-24 sm:py-32">
      <motion.div
        className="pointer-events-none absolute bottom-0 left-1/2 -translate-x-1/2 h-[600px] w-[800px] rounded-full bg-blue-700/[0.09] blur-[130px]"
        style={{ y: glowY }}
      />
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Link href="/pricing" className="flex w-full items-center justify-center rounded-xl border border-white/[0.1] bg-white/[0.04] px-7 py-3.5 text-[14px] font-semibold text-white/65 hover:bg-white/[0.08] hover:text-white transition-all sm:w-auto">
              View pricing
            </Link>
