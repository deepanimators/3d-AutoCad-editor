...
All notable changes to `@aruct/mcp` will be documented in this file.
...
- Initial release.
- `SceneBridge` headless adapter for `@aruct/core` with RAF polyfill so
...
- `aruct-mcp` CLI binary with `--stdio`, `--http --port`, and `--scene`
  flags.
- Local `SqliteSceneStore` backed by built-in SQLite drivers (`bun:sqlite` in
  ...
  `ARUCT_DATA_DIR` / `ARUCT_DB_PATH` configuration for MCP and the editor.
